...
class PriceFinder {
   ...
   *                             item price (a Number).
   */
  findItemPrice(uri, callback, proxy) {
    ...
    return this._pageScrape(site, proxy, (err, pageData) => {
      if (err) {
   ...
   *                             category, and price.
   */
  findItemDetails(uri, callback, proxy) {
    logger.log('findItemPrice with uri: %s', uri);
  ...
  // ============== PRIVATE FUNCTIONS =================
  // ==================================================
  _pageScrape(site, proxy, callback) {
    let pageData;
    ...
    async.whilst(
      ...
      (whilstCallback) => {
        ...
        agent
          ...
          .set('User-Agent', site.getUserAgent() || 'Mozilla')
          .set('Accept-Language', site.getUserLang() || 'en-US')
          .set('Accept', site.getUserAccept() || 'text/html,application/xhtml+xml,application/xml')
          .end((err, response) => {
            if (err) {
