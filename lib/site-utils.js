...
exports.categories = Object.freeze({
  ...
  exports.getUserLang = () => {
    let s = [];
    let languages = [
      'en-GB', 'en-GB', 'en-GB', 'de', 'de', 'de', 'en-US', 'en-US', 'en-US',
      'fr', 'es-ES', 'en', 'en', 'en'
    ];
    let qualities = ['', ';q=0.9', ';q=0.7', ';q=0.4', ';q=0.3']
    const iters = Math.floor(Math.random() * 3) + 1;
    Array.from(Array(iters)).map(() => {
      const l = languages[Math.floor(Math.random()*languages.length)]
      const q = qualities[Math.floor(Math.random()*qualities.length)]
      Math.floor(Math.random() * 3)
      languages = languages.filter(item => item !== l)
      qualities = qualities.filter(item => item !== q)
      s.push(l + q);
    });
    return s.join(', ')
  }

  exports.getUserAccept = () => {
    const accepts = [
      "*/*",
      "*/*",
      "*/*",
